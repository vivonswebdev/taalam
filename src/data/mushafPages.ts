/**
 * Mushaf page mapping: surah number → starting page in the standard Uthmani Mushaf (604 pages).
 * Also includes juz → page mapping.
 */

// Surah number → first page in the Mushaf
...
export const juzStartPage: Record<number, number> = {
...
/**
 * Get surah name info for a given page number
 */
export function getSurahForPage(page: number): number {
  let surah = 1;
  for (const [num, startPage] of Object.entries(surahStartPage)) {
    if (startPage <= page) surah = Number(num);
    else break;
  }
  return surah;
}
...
export function getJuzForPage(page: number): number {
  let juz = 1;
  for (const [num, startPage] of Object.entries(juzStartPage)) {
    if (startPage <= page) juz = Number(num);
    else break;
  }
  return juz;
}
