...
export const DIFFICULTY_CONFIG: DifficultyLevel[] = [
  { levelRange: [1, 10], ops: ["+"], maxVal: 20, xpPerWin: 10, timePerCalc: 5, label: "🟢" },
  { levelRange: [11, 20], ops: ["+", "-"], maxVal: 50, xpPerWin: 15, timePerCalc: 4.5, label: "🟡" },
  { levelRange: [21, 35], ops: ["+", "-", "×"], maxVal: 100, xpPerWin: 25, timePerCalc: 3.5, label: "🟠" },
  { levelRange: [36, 50], ops: ["+", "-", "×", "÷"], maxVal: 100, xpPerWin: 50, timePerCalc: 2.5, label: "🔴" },
];

export function getDifficultyForLevel(level: number): DifficultyLevel {
  return DIFFICULTY_CONFIG.find(d => level >= d.levelRange[0] && level <= d.levelRange[1]) || DIFFICULTY_CONFIG[0];
}
