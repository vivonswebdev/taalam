...
const surahTextCache = new Map<number, string>();

/**
 * TODO résolu : Récupère le texte d'une sourate via l'API Al-Quran Cloud
 * API publique, gratuite, sans clé : https://alquran.cloud/api
 */
export const getSurahText = async (surahNumber: number): Promise<string> => {
  ...
  try {
    ...
    const verses: Array<{ text: string }> = data.data?.ayahs ?? [];
    const fullText = verses.map((a) => a.text).join(' ');

    surahTextCache.set(surahNumber, fullText);
    return fullText;
  } catch (error) {
    const fallback = SURAH_FALLBACK[surahNumber];
    if (fallback) return fallback;
    throw new Error(
      `Impossible de charger la sourate ${surahNumber}. Vérifiez votre connexion.`
    );
  }
};

/**
 * Fallback local pour Juz'Amma (sourates 110-114) — utilisé hors connexion
 */
const SURAH_FALLBACK: Record<number, string> = {
