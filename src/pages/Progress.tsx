              ...
              <BarChart data={chartData}>
                ...
                <Bar dataKey="score" radius={[6, 6, 0, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell key={index} fill={entry.score >= 80 ? "hsl(145, 63%, 42%)" : "hsl(152, 56%, 28%)"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          {progress.surahProgress.length === 0 ? (
            ...
            <div className="space-y-2">
              {progress.surahProgress.map((sp, i) => {
                ...
                return (
                  <motion.div key={sp.surahNumber} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.05 }} className="flex items-center gap-3 bg-card border border-border rounded-xl p-3">
                    <span className="font-arabic text-lg text-primary w-16 text-right">{surah.nameArabic}</span>
                    <div className="flex-1">
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all" style={{ width: `${sp.bestScore}%`, backgroundColor: sp.bestScore >= 80 ? "hsl(145, 63%, 42%)" : "hsl(152, 56%, 28%)" }} />
                      </div>
                    </div>
