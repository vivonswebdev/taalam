...
function VibrantCard({ emoji, title, desc, gradient, loop, onClick }: {
  emoji: string; title: string; desc: string; gradient: string; loop?: boolean; onClick?: () => void;
}) {
  const { t } = useLanguage();
  return (
    <button
      onClick={onClick}
      className={`relative w-full flex flex-col items-center justify-center gap-1 rounded-2xl p-3 bg-gradient-to-br ${gradient} shadow-lg shadow-black/10 min-h-[100px] overflow-hidden transition-transform active:scale-95`}
    >
      <span className="absolute top-1.5 right-2 text-sm opacity-40">✨</span>
      ...
      <span className="text-[11px] font-bold text-white drop-shadow leading-tight text-center">{title}</span>
      <span className="text-[8px] text-white/75 font-medium leading-tight text-center line-clamp-2">{desc}</span>
    </button>
  );
}
...
function RecentMoodsSection() {
  ...
  return (
    <div className="px-4 pt-3 pb-1 relative z-10">
      <p className="text-xs font-bold text-white/50 uppercase tracking-wider px-1 mb-2">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar">
        {items.map(item => (
          <button
            ...
            className="shrink-0 flex items-center gap-1.5 rounded-full px-3 py-1.5 bg-white/[0.07] backdrop-blur-md border border-white/15 hover:border-white/30 transition-colors"
          >
            <span className="text-base">{item.emoji}</span>
            <span className="text-xs font-medium text-white/80 whitespace-nowrap">{item.title}</span>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen pb-24 bg-gradient-to-b from-[hsl(260,50%,12%)] via-[hsl(240,40%,18%)] to-[hsl(220,35%,10%)]">
      ...
      <div className="flex items-center gap-3 px-4 pt-6 pb-4 relative z-10">
        <button onClick={() => navigate(-1)} className="p-2 rounded-xl bg-white/[0.07] backdrop-blur-md border border-white/15">
          <ArrowLeft size={20} className="text-white/70" />
        </button>
        <div>
          <h1 className="text-xl font-extrabold text-white">❤️ {t("moods.title")}</h1>
          <p className="text-xs text-white/50 mt-0.5">{t("moods.subtitle")}</p>
        </div>
      </div>
      ...
      {/* ═══ MALADIES ═══ */}
      {activeTab === "maladies" && (
        <div className="grid grid-cols-3 gap-3 p-4 relative z-10">
          {maladiesPresets.map((m) => (
            <VibrantCard
              key={m.id}
              emoji={m.emoji}
                ...
                <button
                  ...
                  className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                    athkarFilter === f.id
                      ...
                      : "bg-white/[0.05] text-white/50 border-white/10 hover:border-white/25"
                  }`}
                >
                ...
                {coreFiltered.length > 0 && (
                  <>
                    <div className="px-4 pt-2 pb-1">
                      <h2 className="text-sm font-bold text-white/80">📿 {t("athkar.coreTitle" as any)}</h2>
                    </div>
                ...
                {situationFiltered.length > 0 && (
                  <>
                    <div className="px-4 pt-3 pb-1">
                      <h2 className="text-sm font-bold text-white/80">🗂️ {t("athkar.situationsTitle" as any)}</h2>
                      <p className="text-[10px] text-white/40 mt-0.5">{t("athkar.situationsSubtitle" as any)}</p>
                    </div>
