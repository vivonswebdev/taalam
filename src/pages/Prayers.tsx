      ...
      <div className="px-6 space-y-5">
        ...
        {displayCity ? (
          <div className="flex items-center justify-between bg-card border border-border rounded-xl px-3 py-2.5">
            ...
            <button onClick={() => navigate("/prayer-settings?focus=city")} className="text-[10px] font-semibold text-primary bg-primary/10 px-2 py-1 rounded-full">
          ...
          <div className="bg-card border border-border rounded-xl px-3 py-3 flex items-center justify-between">
            ...
            <button onClick={handleDetectCity} className="flex items-center gap-1 text-xs font-semibold text-primary bg-primary/10 px-3 py-1.5 rounded-full">
              <Navigation size={12} />
        ...
        {nextPrayer && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-primary text-primary-foreground rounded-2xl p-5 text-center">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {PRAYER_NAMES_LIST.map((name, i) => {
                ...
                return (
                  <motion.div key={name} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 + i * 0.05 }}
                    className={`flex items-center justify-between rounded-xl p-3 transition-colors ${isNext ? "bg-primary/10 border border-primary/30" : "bg-muted/50"}`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-lg">{PRAYER_ICONS[name]}</span>
                      <span className={`text-sm font-medium ${isNext ? "text-primary font-bold" : "text-card-foreground"}`}>{t(`prayers.${name.toLowerCase()}` as any)}</span>
                    </div>
                    <span className={`text-sm font-semibold tabular-nums ${isNext ? "text-primary" : "text-muted-foreground"}`}>{times[name]}</span>
                  </motion.div>
                );
              })}
            </div>
        ...
        {/* 4 & 5) Notifications + Boussole — icon buttons that expand */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid grid-cols-2 gap-3">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-2 rounded-2xl p-4 border transition-colors ${openPanel === "notif" ? "bg-primary/10 border-primary/30" : "bg-card border-border"}`}
          >
            <span className="text-3xl">🔔</span>
            <p className="text-xs font-bold text-foreground">{t("prayers.notif.title")}</p>
          </button>

          {/* Qibla button */}
          <button
            onClick={() => setOpenPanel(openPanel === "qibla" ? null : "qibla")}
            className={`flex flex-col items-center gap-2 rounded-2xl p-4 border transition-colors ${openPanel === "qibla" ? "bg-primary/10 border-primary/30" : "bg-card border-border"}`}
          >
            <span className="text-3xl">🧭</span>
            <p className="text-xs font-bold text-foreground">{t("prayers.qibla")}</p>
          </button>
        </motion.div>
        ...
        <AnimatePresence>
          {openPanel === "notif" && (
            ...
            >
              ...
              {!notif.supported ? (
                <div className="flex items-center gap-2 text-xs text-muted-foreground"><AlertTriangle size={14} /><span>{t("prayers.notif.unsupported")}</span></div>
                ...
                <>
                  ...
                  >
                    ...
                    <div className={`w-10 h-6 rounded-full transition-colors relative ${notif.config.enabled ? "bg-primary" : "bg-muted"}`}>
                      <motion.div animate={{ x: notif.config.enabled ? 16 : 2 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} className="absolute top-0.5 w-5 h-5 rounded-full bg-card shadow-md" />
                    </div>
                  </button>
                    ...
                    <div className="space-y-1.5">
                      {PRAYER_NAMES_LIST.map((name) => (
                        <button key={name} onClick={() => notif.togglePrayer(name)} className="w-full flex items-center justify-between py-1.5">
                          ...
                          {notif.config.prayers[name] ? <Bell size={14} className="text-primary" /> : <BellOff size={14} className="text-muted-foreground" />}
                        </button>
                      ))}
          ...
          {openPanel === "qibla" && (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                {!permissionGranted ? (
                  <button onClick={requestPermission} className="bg-primary text-primary-foreground px-6 py-3 rounded-xl font-semibold text-sm active:scale-[0.97] transition-transform">
                    {t("prayers.enableCompass")}
                  </button>
                ) : (
                  <div className="relative w-48 h-48">
                    <motion.div className="absolute inset-0 rounded-full border-4 transition-colors duration-500"
                      animate={{ borderColor: isAligned ? "hsl(var(--success))" : "hsl(var(--muted))", boxShadow: isAligned ? "0 0 20px hsl(var(--success) / 0.4), 0 0 40px hsl(var(--success) / 0.15)" : "none" }}
                ...
                {isAligned && permissionGranted && (
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center gap-1.5 mt-3 bg-success/15 text-success rounded-full px-4 py-1.5">
