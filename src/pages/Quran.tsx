          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            {[
              ...
              <motion.button
                ...
                className={`card-shimmer relative overflow-hidden rounded-2xl p-3 flex flex-col gap-1 text-left shadow-sm bg-gradient-to-br ${item.gradient} ${
                  recitationMode === item.mode ? "ring-2 ring-white/40 shadow-lg" : ""
                }`}
              >
                <span className="w-7 h-7 rounded-full bg-white/20 flex items-center justify-center text-base">{item.emoji}</span>
                <span className="text-xs font-bold text-white leading-tight mt-0.5">{item.label}</span>
                <span className="text-[9px] text-white/70 leading-snug line-clamp-2">{item.desc}</span>
              </motion.button>
            ))}
          </motion.div>
