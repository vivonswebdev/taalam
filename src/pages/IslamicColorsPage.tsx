...
export default function IslamicColorsPage() {
  ...
  useEffect(() => {
    setQuestions(shuffle(QUESTIONS).slice(0, TOTAL));
  }, []);

  const handleColorPick = (colorId: string) => {
    if (feedback || current >= questions.length) return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-50 via-purple-50 to-pink-50 dark:from-background dark:via-background dark:to-background flex flex-col">
      <div className="flex items-center gap-3 px-4 pt-6 pb-3">
        ...
        <h1 className="text-sm font-bold text-foreground flex-1">🎨 {t("kidsGames.islamicColors" as any)}</h1>
      ...
      <div className="px-4 mb-4">
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <motion.div className="h-full bg-primary rounded-full" animate={{ width: `${((current + (feedback ? 1 : 0)) / TOTAL) * 100}%` }} />
        </div>
      ...
      {q && (
        <div className="flex-1 flex flex-col items-center justify-center px-6 gap-6">
          {/* Streak */}
          {streak >= 2 && (
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-amber-500/15 px-3 py-1 rounded-full text-xs font-bold text-amber-600 dark:text-amber-400">
              🔥 {streak} combo!
            </motion.div>
          )}
          ...
          <AnimatePresence>
            {feedback && (
              <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="text-xs text-center text-muted-foreground">{q.rule}</motion.p>
            )}
          </AnimatePresence>
